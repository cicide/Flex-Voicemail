...
def route(agi):
    ...
    def processScript(result):
        ...
        if script == 'leave-voicemail':
            f = VmDeposit()
            return f(agi, cidname, cidnum, gid, gext, cuid)
        elif script == 'get-voicemail':
            f = VmRetrieve()
            return f(agi, gid, auth_ext, cuid)
        else:
            log.debug("Unknown AGI call: %s" % script)
            agi.finish()
